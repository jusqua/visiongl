...
void VglNamedWindowList::RefreshAll(int split) {
    ...
    if (split < VGL_MIN_WINDOW_SPLIT || split > VGL_MAX_WINDOW_SPLIT) {
        printf("VglNamedWindowList::RefreshAll(): Error: split = %d not between %d and %d. Using default, %d",
               split,
               VGL_MIN_WINDOW_SPLIT,
               VGL_MAX_WINDOW_SPLIT,
               VGL_DEFAULT_WINDOW_SPLIT);
...
void vglUpload(VglImage* image, int swapRGB) {
    ...
    if (ndim == 3) {
        glTexImage3D(glTarget, LEVEL, internalFormat, image->getWidth(), image->getHeight(), image->getLength(), 0, glFormat, glType, image->ndarray);
    } else {
        glTexImage2D(glTarget, LEVEL, internalFormat, ipl->width, ipl->height, 0, glFormat, glType, ipl->imageData);
    }
    ...
    if (image->fbo == static_cast<GLuint>(-1)) {
        ...
        if (glTarget == GL_TEXTURE_3D) {
            ...
            glFramebufferTexture3DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, glTarget, image->tex, 0, layer);
        } else {
            glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, glTarget, image->tex, 0);
        }
        CHECK_FRAMEBUFFER_STATUS()
        ERRCHECK()
    }
