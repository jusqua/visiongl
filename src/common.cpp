...
int iplFindBitsPerSample(int depth) { return depth & 255; }
...
int iplFindWidthStep(int depth, int width, int channels /*= 1*/) {
  ...
  if (bpp < 8) {
    fprintf(stderr,
            "%s:%s: Error: bits per pixel = %d < 8 and != 1. Image depth may "
            "be wrong.\n",
...
IplImage *iplCreateImage(CvSize size, int depth, int channels) {
  ...
  /* TODO: remove all instances of this and replace with iplFindBitsPerSample
  and iplFindWidthStep size_t bytesPerPixel = depth & 255; bytesPerPixel /= 8;
  if (bytesPerPixel < 1)
    bytesPerPixel = 1;
...
void iplCvtColor(IplImage *src, IplImage *dst, int code) {
  ...
  case CV_GRAY2BGRA:
    // case CV_GRAY2RGBA: //9
    {
      if (src->nChannels != 1) {
        fprintf(stderr,
                "%s:%s: Error: src image should have 1 channel but has %d "
                "channels\n",
  ...
  case CV_RGB2GRAY: // 7
  {
    if (src->nChannels != 3) {
      fprintf(stderr,
              "%s:%s: Error: src image should have 3 channels but has %d "
              "channels\n",
              __FILE__, __FUNCTION__, src->nChannels);
      exit(1);
    }
  ...
  case CV_RGBA2GRAY: // 11
  {
    if (src->nChannels != 4) {
      fprintf(stderr,
              "%s:%s: Error: src image should have 4 channels but has %d "
              "channels\n",
              __FILE__, __FUNCTION__, src->nChannels);
      exit(1);
    }
  ...
  if (dst) {
    ...
    case CV_GRAY2BGRA:
      // case CV_GRAY2RGBA: //9
      {
        if (dst->nChannels != 4) {
          fprintf(stdout,
                  "%s:%s: Error: dst image should have 4 channels but has %d "
                  "channels\n",
    ...
    case CV_GRAY2BGR:
      // case CV_GRAY2RGB: //8
      {
        if (dst->nChannels != 3) {
          fprintf(stdout,
                  "%s:%s: Error: dst image should have 3 channels but has %d "
                  "channels\n",
                  __FILE__, __FUNCTION__, dst->nChannels);
          exit(1);
        }
    ...
    case CV_RGBA2GRAY: // 11
    {
      if (dst->nChannels != 1) {
        fprintf(stdout,
                "%s:%s: Error: dst image should have 1 channel but has %d "
                "channels\n",
                __FILE__, __FUNCTION__, dst->nChannels);
        exit(1);
      }
        ...
        for (int j = 0; j < w; j++) {
          ...
          for (int k = 0; k < dstNChan; k++) {
            ...
            if (dstIChan[k] == 3 && srcIChan[k] == -1) {
              ...
              switch (d) {
              case 1:
                ((uint8_t *)dst->imageData)[dstIdx] =
                    ((uint8_t *)src->imageData)[srcIdx];
                break;
              case 2:
                ((uint16_t *)dst->imageData)[dstIdx] =
                    ((uint16_t *)src->imageData)[srcIdx];
                break;
              case 4:
                ((uint32_t *)dst->imageData)[dstIdx] =
                    ((uint32_t *)src->imageData)[srcIdx];
                break;
              case 8:
                ((uint64_t *)dst->imageData)[dstIdx] =
                    ((uint64_t *)src->imageData)[srcIdx];
                break;
              }
            }
          }
...
/** Skip comments inside PPM/PGM/PBM file.
    ...
    Based on code from Along the Pathway blog, Reading/Writing PGM Files in C,
   Posted on March 4, 2010 by ugurkoltuk. Original version had a bug, not
   working on big lines.
...
IplImage *iplLoadPgm(char *filename) {
  ...
  if (id == 4) {
    ...
    if (L == 255) {
      ...
      fprintf(stderr,
              "%s: %s: Error loading PGM file %s. Dynamic range = %d "
              "unsupported.\n",
...
IplImage *iplLoadImage(char *filename,
  ...
  if (strcmp(ext, ".PGM") == 0 || strcmp(ext, ".PPM") == 0 ||
...
#else
    fprintf(stderr,
            "%s:%s: Error: TIFF format unsupported. Please recompile using "
            "WITH_TIFF = 1.\n",
            __FILE__, __FUNCTION__);
    exit(1);
#endif
    ...
    fprintf(stderr,
            "%s:%s: Error: extension %s unsupported. You may try to recompile "
            "using WITH_OPENCV = 1 or use iplLoadPgm instead.\n",
  ...
  if (iplImage) {
    ...
    case CV_LOAD_IMAGE_GRAYSCALE: {
      ...
      default:
        fprintf(stderr,
                "%s:%s: Error: loaded image has number of channels = %d "
                "unsupported by iplCvtColor. You may try to recompile using "
                "WITH_OPENCV = 1.\n",
...
unsigned byte or short, or PBM file. Can be used with ipl or ndarray type of
image.
*/
int iplGenericSavePgm(char *filename, char *buf, int w, int h, int widthStep,
                      int c, int bps) {
  if (buf == NULL) {
  ...
  if (bps == 1) {
    ...
    fprintf(stderr,
            "%s: %s: Error saving PGM file %s. Unsupported number of channels "
            "= %d.\n",
  ...
  if ((bps != 1) && (bps != 8) && (bps != 16)) {
    fprintf(stderr,
            "%s: %s: Error saving PGM file %s. Unsupported pixel depth = %d.\n",
...
int iplSaveImage(char *filename, IplImage *image, int *params /*=0*/) {
  ...
  if (strcmp(ext, ".PGM") == 0 || strcmp(ext, ".PPM") == 0 ||
    ...
    fprintf(stderr,
            "%s:%s: Error: extension %s unsupported. You may try to recompile "
            "using WITH_OPENCV = 1 or use iplSavePgm instead.\n",
            __FILE__, __FUNCTION__, ext);
    exit(1);
  }
...
void cvReleaseImage(IplImage **p_ipl) { iplReleaseImage(p_ipl); }

IplImage *cvCreateImage(CvSize size, int depth, int channels) {
  return iplCreateImage(size, depth, channels);
}

IplImage *cvCopy(IplImage *src, IplImage *dst) { return iplCopy(src, dst); }

void cvCvtColor(IplImage *src, IplImage *dst, int code) {
  iplCvtColor(src, dst, code);
}

IplImage *cvLoadImage(char *filename,
                      int iscolor /*= CV_LOAD_IMAGE_UNCHANGED*/) {
