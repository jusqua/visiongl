...
ParseOpts(Opts *opts, int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "h", longOptions, NULL)) != EOF) {
        switch (opt) {
        case 'h':
        default:
            Usage(argv[0]);
...
void PrintSupportedFormats(cl_device_id device, cl_mem_object_type imageType) {
    ...
    clGetSupportedImageFormats(cxGPUContext, CL_MEM_READ_ONLY, imageType, 0, NULL, &uiNumSupportedFormats);
    cl_image_format *ImageFormats = new cl_image_format[uiNumSupportedFormats];
    clGetSupportedImageFormats(cxGPUContext, CL_MEM_READ_ONLY, imageType, uiNumSupportedFormats, ImageFormats, NULL);
    ...
    for (unsigned int i = 0; i < uiNumSupportedFormats; i++) {
        printf("  %-6u%-22s%-22s\n", (i + 1), CLImageFormatString(ImageFormats[i].image_channel_order), CLImageFormatString(ImageFormats[i].image_channel_data_type));
    }
    printf("\n");
    delete[] ImageFormats;
}
