...
VglSimpleBGModel::VglSimpleBGModel(VglImage *img_in, float std_thresh,
                                   int window_size, int num_training_images) {
  ...
  background32 = vglCreateImage(cvSize(width, height), IPL_DEPTH_32F,
                                nChannels); // should be float
  variance32 = vglCreateImage(cvSize(width, height), IPL_DEPTH_32F,
                              nChannels); // should be float
...
VglSimpleBGModel::~VglSimpleBGModel() {
  ...
  if (sobel)
    vglReleaseImage(&sobel);
  if (color)
    vglReleaseImage(&color);
  if (grayx)
    vglReleaseImage(&grayx);
  if (grayy)
    vglReleaseImage(&grayy);
}
...
void VglSimpleBGModel::DetectFGSimpleBGModel(VglImage *img_in) {
  ...
  vglDetectFGSimpleBGModel(img_in, background32, variance32, foreground,
                           std_thresh);
}
/*
...
void VglSimpleBGModel::UpdatePartialSimpleBGModel(VglImage *img_in) {
  ...
  vglUpdatePartialSimpleBGModel(img_in, foregroundClose, background32,
                                variance32, weight);
}
...
void VglSimpleBGModel::UpdateSimpleBGModel(VglImage *img_in) {
  if (curr_training_image < num_training_images) {
    ...
    // Devido a incorporacao pelo fundo adicionada dilatacao inexistente no
    // SimpleBgModel
...
void VglSimpleBGModel::UpdateSimpleBGModelXGY(VglImage* img_in){
  if (!sobel)
          sobel = cvCreateImage(cvSize(img_in->width, img_in->height),
IPL_DEPTH_16S, img_in->nChannels); if (!color) color =
cvCreateImage(cvSize(img_in->width, img_in->height), IPL_DEPTH_8U,
img_in->nChannels); if (!grayx) grayx = cvCreateImage(cvSize(img_in->width,
img_in->height), IPL_DEPTH_8U, 1); if (!grayy) grayy =
cvCreateImage(cvSize(img_in->width, img_in->height), IPL_DEPTH_8U, 1);
...
void VglSimpleBGModel::UpdateSimpleBGModelRGL(VglImage* img_in){
  if (!sobel)
          sobel = cvCreateImage(cvSize(img_in->width, img_in->height),
IPL_DEPTH_16S, img_in->nChannels); if (!color) color =
cvCreateImage(cvSize(img_in->width, img_in->height), IPL_DEPTH_8U, 3); if
(!grayx) grayx = cvCreateImage(cvSize(img_in->width, img_in->height),
IPL_DEPTH_8U, 1);
